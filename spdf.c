...
#define PORT 6543        // Port for Spdf server
...
// Function to create a tar file of .pdf files in ~/spdf
void create_tar_of_pdf_files(const char *tar_filename)
{
    char command[BUFFER_SIZE];
    snprintf(command, sizeof(command), "find /home/parmar8a/spdf -type f -name '*.pdf' | xargs tar -cvf %s", tar_filename);
...
// Function to get the list of .pdf files from the specified directory
...
void handle_display_command(int client_socket, const char *pathname)
{
    ...
    snprintf(full_path, BUFFER_SIZE, "/home/parmar8a/spdf/%s", pathname + 8);
...
void handle_client(int client_socket)
{
    ...
    if (strcmp(command, "ufile") == 0)
    {
        ...
        snprintf(full_destination_path, BUFFER_SIZE, "/home/parmar8a/spdf/%s", destination_path + 8);
    ...
    else if (strcmp(command, "dfile") == 0)
    {
        ...
        char full_path[BUFFER_SIZE];
        snprintf(full_path, BUFFER_SIZE, "/home/parmar8a/spdf/%s", filename + 8);
    ...
    else if (strcmp(command, "dtar") == 0)
    {
        ...
        if (filename && strcmp(filename, ".pdf") == 0)
        {
...
int main()
{
    ...
    // Creating the socket file descriptor for the Spdf server
    ...
    // Configuring the address structure for the Spdf server
    ...
    // Start listening for incoming connections on the Spdf server
